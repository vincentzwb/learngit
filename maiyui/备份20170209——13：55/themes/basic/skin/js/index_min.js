...
function section(){
	...
	$('#contain').fullpage({
		...
		afterRender: function() {
			...
			$(".switch_wrap").slimScroll({
				height:'100%'
			});
			
			$(".section3 .switch").mouseenter(function(){
				$.fn.fullpage.setAllowScrolling(false);
			})
			$(".section3 .switch").mouseleave(function(){
				$.fn.fullpage.setAllowScrolling(true);
			})
			
			var switch_length=$(".section3 .switch_ele").length;
			
			$(".section3 .switch_ele").on("click",function(){
				var switch_index=$(this).index();
				
				if(!$(this).hasClass("selected")){
					$(".section3 .switch_ele").removeClass("selected");
					$(this).addClass("selected")
					
					$(".slider_02").trigger("slideTo",switch_index)
				}
